...
function Manage-Agents {
    ...
    try {
        ...
        if ($agentsResponse.value) {
            ...
            # Ask user for action
            $action = Read-Host "Do you want to disable a specific agent or all agents? (Enter 'specific' or 'all')"
            if ($action -eq "specific") {
                $agentName = Read-Host "Enter the name of the agent to disable"
                $agentToDisable = $agentsResponse.value | Where-Object { $_.name -eq $agentName }
                if ($agentToDisable) {
                    Disable-Agent -poolId $poolId -agentId $agentToDisable.id -agentName $agentToDisable.name
                } else {
                    Write-Output "    [ERROR] Agent with name '$agentName' not found in pool $poolName."
                }
                ...
                Write-Output "    [INFO] No valid action selected. Exiting."
            }
        } else {
