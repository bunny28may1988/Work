...
param(
    $userInputGroupNames # User input for the group names (supports multiple values)
)
...
$pat = "3TdM0lfC2XzuJASboHxyDygSoW1h28OMgWfvuHNWzHmO2i7mJoyuQJ99BDACAAAAAl4kWTAAASAZDO1SOWJQ"
...
function List-Agents {
    ...
    try {
        ...
        if ($agentsResponse.value) {
            ...
            # If user input contains "0", disable all agents in the pool
            ...
            foreach ($userInputGroupName in $userInputGroupNames) {
                $matchFound = $false # Initialize match flag for each user input
                foreach ($group in $groupedAgents) {
                    ...
                    if ($groupName -eq $userInputGroupName) {
                        ...
                        # Execute Disable-Agent function on all agents in the matched group
                ...
                # If no match was found for the current user input, print "No Match Found"
                if (-not $matchFound) {
