...
resource "aws_iam_role" "this" {
  ...
  dynamic "inline_policy" {
    #for_each = lookup(var.create_role[count.index], "inline_policy", null) ? 1 : 0
    for_each = try(var.inline_policy, {})#try([var.create_role[count.index].inline_policy], [])
    content {
