...
chmod -R 600 /etc/kubernetes/kubelet
chmod 600 /etc/kubernetes/kubelet/kubelet-config.json
chown root:root /etc/kubernetes/kubelet/kubelet-config.json
sed -e '/--hostname-override=/ s/^#*/#/' -i /etc/eks/bootstrap.sh
sed -i 'N;/\n}/{s//,&/;P;s/".*,/"eventRecordQPS": 0/};P;D' /etc/kubernetes/kubelet/kubelet-config.json
sed -i 'N;/\n}/{s//,&/;P;s/".*,/"RotateCertificate": true/};P;D' /etc/kubernetes/kubelet/kubelet-config.json
